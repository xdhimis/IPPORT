...
# Add temporal and paper-inspired features for each metric
...
# Multi-feature Isolation Forest (using z-normalized features)
z_feats = [f'z_{feat}' for feat in all_features] + [f'z_z_{metric}' for metric in metrics]  # Include double-z if needed, but skip
model_multi = IsolationForest(contamination=0.01, random_state=42)
model_multi.fit(df_normal[z_feats])
df_problem['anomaly_multi'] = model_multi.predict(df_problem[z_feats])
df_problem['anomaly_multi'] = df_problem['anomaly_multi'].map({1: 0, -1: 1})
df_problem['score_multi'] = model_multi.decision_function(df_problem[z_feats])

# 3-sigma flags per metric
...
# Infer dependency graph from normal correlations (aggregate metrics or use one, e.g., qps)
pivot_normal = df_normal.pivot(index='timestamp', columns=group_key, values='qps').fillna(0)  # Use QPS for graph
...
# Directionality via Granger causality
directions = {}
...
# Ranking root causes (corr to overall system metric, e.g., total EPS as business metric)
...
pivot_problem = df_problem.pivot(index='timestamp', columns=group_key, values='qps').fillna(0)
